...
一个轻量的 Codex extension，用于把本机的 pi、Google Antigravity (`agy`)、Claude Code (`claude`) 或任意兼容 CLI 配置成 Codex 可调用的 sub agent。
...
默认 provider 使用 `pi -p {prompt}`、`agy --prompt {prompt}` 和 `claude -p {prompt}`。可通过配置覆盖命令和参数：
